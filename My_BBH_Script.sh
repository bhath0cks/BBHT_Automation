...
subfinder -d ${Domain} -silent -nc | httpx -mc 200,301,302,401,403 -silent -nc | tee $output/${Domain}_Subdomain.txt
...
uncover -shodan 'ssl.cert.subject.CN:"${Domain}" 200' -shodan-idb 'ssl.cert.subject.CN:"${Domain}" 200' -silent -nc | httpx -mc 200 -silent -nc | tee -a $output/${Domain}_Host_Enumeration.txt
...
cat $output/${Domain}_Subdomain.txt | dnsx -a -resp-only -t 1000 -silent -o $output/${Domain}_ip.txt
...
echo ""
for ip in $(cat $output/${Domain}_ip_non_cdn.txt);do echo $ip && ffuf -w $output/${Domain}_Subdomain.txt -u http://$ip -H "Host: FUZZ" -s -mc 200; done | tee -a $output/${Domain}_OriginIP_Final.txt
echo ""
