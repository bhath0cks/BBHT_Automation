...
echo "${BLUE} =============================================================${RESET}"
echo "${BLUE} -->            RECONNAISANCE AUTOMATION FOR BBH          <-- ${RESET}"
...
subscraper ${Domain} --censys-id ${Censys_ID_key} --censys-secret ${Censys_Secret_key} -r $output/${Domain}_subscraper.txt
github-subdomains -d ${Domain} -t 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 -o $output/${Domain}_github.txt

echo ""
cat $output/${Domain}_subfinder.txt > $output/${Domain}_subdomain.txt
echo ""
cat $output/${Domain}_amass.txt | anew $output/${Domain}_subdomain.txt
echo ""
cat $output/${Domain}_assetfinder.txt | anew $output/${Domain}_subdomain.txt
echo ""
cat $output/${Domain}_subscraper.txt | anew $output/${Domain}_subdomain.txt
echo ""
cat $output/${Domain}_github.txt | anew $output/${Domain}_subdomain.txt
echo ""

altdns -i $output/${Domain}_subdomain.txt -o $output/data_output -w ~/tools/altdns/words.txt -r -s $output/${Domain}_altdns.txt -t 100

echo ""
cat $output/${Domain}_subdomain.txt > $output/${Domain}_subdomain_AIO.txt
echo ""
cat $output/${Domain}_altdns.txt | anew $output/${Domain}_subdomain_AIO.txt
echo ""

rm $output/${Domain}_subfinder.txt $output/${Domain}_amass.txt $output/${Domain}_assetfinder.txt $output/${Domain}_subscraper.txt $output/${Domain}_github.txt $output/${Domain}_altdns.txt 
...
cat $output/${Domain}_subdomain_AIO.txt | naabu -o $output/${Domain}_naabu.txt
...
cat $output/${Domain}_probed.txt | nuclei -t ~/nuclei-templates/ | discat -1 -u ${Discord}
...
read -p "${YELLOW} [?] Please Input your Censys_Secret Key: " Censys_Secret_key
echo ""

read -p "${YELLOW} [?] Please input your Discord Webhook URL for Notification : " Discord
echo ""
